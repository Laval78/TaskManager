...
namespace TaskManager.WebApi.Controllers
{
    ...
    public class TaskListUserController : ControllerBase
    {
        ...
        {
            if (Request.Headers.TryGetValue("User-Id", out var userIdStr) &&
        ...
        public async Task<IActionResult> AddUserToTaskList(int taskListId, [FromBody] PostTaskListUserDto dto)
        {
            ...
            var success = await _taskListUserService.AddUserToTaskListAsync(taskListId, dto, currentUserId);
            if (!success)
        ...
        {
            var currentUserId = GetCurrentUserId();
            var result = await _taskListUserService.GetUsersForTaskListAsync(taskListId, currentUserId);
        ...
        public async Task<IActionResult> RemoveUserFromTaskList(int taskListId, int userIdToRemove)
        {
            ...
            var success = await _taskListUserService.RemoveUserFromTaskListAsync(taskListId, userIdToRemove, currentUserId);
            if (!success)
